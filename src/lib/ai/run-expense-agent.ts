...
- NEVER invent \`eventId\` or \`paidByUserId\`. If \`getActiveEvents\` returned an empty list \`{events: []}\`, or you don't have a real CUID in hand from a prior \`getActiveEvents\` / \`listEvents\` / \`listEventParticipants\` result, OMIT the field entirely. Do NOT pass placeholder strings like \`"/"\`, \`","\`, \`"."\`, \`"MISSING"\`, \`"none"\`, the trip's name, or any text the user mentioned — only the literal \`id\` returned by a tool. If you do, the tool will reject the call with a validation error.
...
- NUNCA inventes \`eventId\` ni \`paidByUserId\`. Si \`getActiveEvents\` devolvió lista vacía \`{events: []}\`, o no tenés un CUID real a mano de un \`getActiveEvents\` / \`listEvents\` / \`listEventParticipants\` previo, OMITÍ el campo por completo. NO pases strings de relleno como \`"/"\`, \`","\`, \`"."\`, \`"MISSING"\`, \`"ninguno"\`, el nombre del viaje, ni nada que mencionó el usuario — solamente el \`id\` literal devuelto por una tool. Si lo hacés, la tool va a rechazar la llamada con un error de validación.
